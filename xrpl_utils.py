...
class XRPLClient:
    ...
    async def get_account_nfts(self, address: str, limit: int = 400) -> Optional[List[Dict[str, Any]]]:
        """
        Get NFTs owned by an account.
        
        Automatically paginates through all results using the marker field.
        
        Args:
            address: The XRP wallet address
            limit: Max NFTs per request page (server max is typically 400)
        ...
        try:
            while True:
                ...
                request = AccountNFTs(**request_kwargs)
                response = await self.client.request(request)
                ...
                page_nfts = response.result.get("account_nfts", [])
